...
- **一次按键送的是变动的块,不是整篇文档。** daemon 留着上一次送出的页面,逐块
  比较,推一个 splice——和终端预览那条路上的 patch 同一个形状、同一个理由。
  在一篇 1,800 行的中英混排文档上实测,段落里打一个字:

  | 编辑动作 | 之前 | 现在 |
  |:--|--:|--:|
  | 行内打字 | 264,130 B | 327 B |
  | 按 `<CR>` 插入一行 | 264,130 B | 111 B |
  | 在开头 `dd` | 264,130 B | 112 B |
  | 末尾追加 | 264,130 B | 238 B |
  | 改一个标题(目录也变) | 264,130 B | 6,261 B |

  比较块的时候**排除 data-line**,这是关键:按一次回车会让下面每个块的行号都
  变、别的什么都没变,照字面比就会判定"整篇都变了"——这正是当年 `Line` 不比较
  自己 `src` 的那个坑([`SrcPatch`] 的注释里写着)。行号的修正跟着 patch 走,
  一个整数,页面收到后连嵌套元素(引用里的段落、列表项、表格行)一起改。

- 页面自己验两件事:patch 报的是不是它手上那一版,以及拼接完块数对不对。任一
  不对就重新加载自己——这是它唯一能"要一份新的"的办法,并且带 30 秒的节流,
  不会转圈。因此不再需要定期整篇重发(那在长文档上是每 32 次按键 264 KB)。

- 含有裸 HTML 块的文档一律整篇推送。裸 HTML 是原样输出的,可以在一个块里开标签、
  三个块之后才闭合,块和元素就对不上了。不会出错,只是没有省。

- 页面目录不再携带行号:它从来没被用过,而每次插入行都会让 60 条目录全部重发
  (6.8 KB)。

- 打开页面不再传两遍文档。页面本身就带着正文,事件流从前还会在连上的第一时间
  再送一份(长文档 263 KB);现在页面把手上的版本号告诉 `/events`,一致就不送。

- KaTeX 的样式表改成非阻塞加载(`media="print"` + `onload`)。此前它是 `<head>`
  里一个普通的 `<link>`:CDN 慢或不通时,浏览器会对着**空白页**等它——为了排版
  一篇可能根本没有公式的文档。
